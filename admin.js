...
function initAdmin() {
  ...
  // ── Swap Mode ──
  document.getElementById('toggleSwapModeBtn')?.addEventListener('click', () => {
    swapModeActive = true;
    updateSwapModeUI();
  ...
  // Export dropdown
  const exportContainer = document.querySelector('.top-actions .export-dropdown');
  if (exportContainer) {
    ...
    document.addEventListener('click', (e) => { if (!exportContainer.contains(e.target)) menu?.classList.add('hidden'); });
  }
...
function switchTab(name) {
  ...
  const titles = {
    dashboard: 'Dashboard', teams: 'Kelola Tim', schedule: 'Jadwal Grup',
    results: 'Input Hasil', playoff: 'Playoff Bracket', timeline: 'Timeline Turnamen',
    standings: 'Klasemen Grup', settings: 'Pengaturan'
  };
...
function renderUpcomingMini() {
  ...
  if (finalMatch?.played && finalMatch?.winner) {
    ...
    const bronze3rd = bronzeMatch?.played && bronzeMatch?.winner ? teamById(bronzeMatch.winner, data) : null;
    const bronze4th = bronzeMatch?.played && bronzeMatch?.winner
      ? teamById(bronzeMatch.winner === bronzeMatch.teamA ? bronzeMatch.teamB : bronzeMatch.teamA, data)
      : null;
  ...
  if (!list.length && !data.bracket?.r16?.length) {
    wrap.innerHTML = '<div class="empty-state">📭 Belum ada pertandingan. Tambah tim dan generate jadwal.</div>';
    return;
  }
  if (!list.length) {
    wrap.innerHTML = '<div class="empty-state">✅ Semua match grup sudah selesai</div>';
    return;
  }
...
function renderAdminTeams() {
  ...
  if (!data.teams.length) {
    wrap.innerHTML = '<div class="empty-state">👥 Belum ada tim. Klik "+ Tambah Tim" untuk menambahkan tim pertama.</div>';
    return;
  }
...
function openTeamModal(id) {
  ...
  data.teams.forEach(team => {
    if (team.id !== id && groupCounts[team.group] !== undefined) groupCounts[team.group]++;
  });
  ...
  const groupOptions = GROUPS.map(g => {
    ...
    const label    = isFull && !isSelected ? `Grup ${g} (penuh)` : `Grup ${g} (${count}/4)`;
...
window.saveTeam = function (id) {
  ...
  const duplicate = data.teams.find(t => t.name.toLowerCase() === name.toLowerCase() && t.id !== id);
  if (duplicate) { toast(`Nama tim "${name}" sudah digunakan`, 'error'); return; }

  if (id) {
    const t = data.teams.find(x => x.id === id);
    const oldGroup = t.group;
    if (group !== oldGroup) {
      const newGroupCount = data.teams.filter(x => x.group === group && x.id !== id).length;
      if (newGroupCount >= 4) { toast(`Grup ${group} sudah penuh (maks 4 tim)`, 'error'); return; }
    }
    Object.assign(t, { name, tag, group });
  } else {
    const groupCount = data.teams.filter(x => x.group === group).length;
    if (groupCount >= 4) { toast(`Grup ${group} sudah penuh (maks 4 tim)`, 'error'); return; }
    data.teams.push({
      id: 't' + Date.now(), name, tag, group,
...
function renderAdminSchedule() {
  ...
  if (filterGroup !== 'all') filteredMatches = data.matches.filter(m => m.group === filterGroup);
  if (!filteredMatches.length) {
    wrap.innerHTML = '<div class="empty-state">📅 Tidak ada jadwal. Tambah tim lalu generate jadwal.</div>';
    return;
  }
  wrap.innerHTML = filteredMatches.slice(0, 80).map(m => matchCardListHtml(m, data)).join('');
}
...
function matchCardListHtml(match, data) {
  ...
  const scoreText   = match.played ? `${match.scoreA} - ${match.scoreB}` : '- : -';
...
window.openGroupMatchupEditModal = function (matchId) {
  ...
  const teamOptions = (currentId) => groupTeams
    .map(t => `<option value="${t.id}" ${t.id === currentId ? 'selected' : ''}>${t.name} (${t.tag})</option>`)
    .join('');

  const hasResult = m.played;

  openModal(`✏️ Edit Matchup — Grup ${m.group}`, `
...
window.saveGroupMatchup = function (matchId) {
  ...
  const duplicate = data.matches.find(x =>
    x.id !== matchId &&
    x.group === m.group &&
    ((x.teamA === newA && x.teamB === newB) || (x.teamA === newB && x.teamB === newA))
  );
  if (duplicate) { toast('Matchup ini sudah ada di jadwal grup yang sama', 'error'); return; }
  ...
  if (changed && m.played) {
    ...
    m.scoreA = null;
    m.scoreB = null;
    toast('Matchup diperbarui. Hasil match direset karena ada perubahan tim.', 'warning');
  } else {
...
function renderAdminResults() {
  ...
  if (filterGroup !== 'all') filteredMatches = data.matches.filter(m => m.group === filterGroup);
  if (!filteredMatches.length) {
    wrap.innerHTML = '<div class="empty-state">⚽ Tidak ada pertandingan. Tambah tim dan generate jadwal terlebih dahulu.</div>';
    return;
  }
  wrap.innerHTML = filteredMatches.slice(0, 80).map(m => {
    const a = teamById(m.teamA, data);
    const b = teamById(m.teamB, data);
    return `
      <div class="match-card-list">
...
window.saveBO1 = function (id) {
  ...
  if (isNaN(a) || isNaN(b)) { toast('Masukkan skor yang valid (0 atau 1)', 'error'); return; }
  if (![0, 1].includes(a) || ![0, 1].includes(b) || a === b) { toast('Skor BO1 hanya 1-0 atau 0-1', 'error'); return; }
...
function renderAdminStandings() {
  ...
  if (!data.teams.length) {
    wrap.innerHTML = '<div class="empty-state">📊 Belum ada tim untuk ditampilkan.</div>';
    return;
  }
  ...
  wrap.innerHTML = html || '<div class="empty-state">📊 Belum ada data klasemen.</div>';
}
...
window.openMoveSingleTeamModal = function(teamId) {
  ...
  // Blok warna helper
  const blockColor = g => ['A','B','C','D'].includes(g)
  ...
  openModal(`↗ Pindah Tim — ${team.name}`, `
...
window.doMoveTeam = function(teamId, newGroup) {
  ...
  const newGroupCount = data.teams.filter(t => t.group === newGroup && t.id !== teamId).length;
  if (newGroupCount >= 4) {
  ...
  const newGroupTeams = data.teams.filter(t => t.group === newGroup && t.id !== teamId);
  let mid = Date.now();
  ...
  if (resetCount > 0) msg += ` ${resetCount} match lama dihapus, match baru dibuat.`;
...
function bracketMatchHtml(m, data, key, extraClass = '') {
  if (!m || !m.id) return `<div class="bracket-match ${extraClass}" style="opacity:0.3">
...
function openMatchupEditModal(matchId) {
  ...
  if (!m.teamA && !m.teamB) {
    toast('Match ini belum memiliki tim (menunggu round sebelumnya)', 'warning');
    return;
  }
  ...
  const teamOptions = (currentId) => data.teams
  ...
  openModal('✏️ Edit Matchup Bracket', `
    <div class="matchup-edit-wrap">
...
window.doSwapMatchup = function (matchId) {
  ...
  if (m.played) {
    ...
    m.scoreB = null;
    m.winner = null;
    toast('Posisi ditukar. Hasil match direset karena ada perubahan matchup.', 'warning');
  } else {
...
function openBO3Modal(matchId, data) {
  ...
  const m   = all.find(x => x.id === matchId);
  if (!m) return;
  if (!m.teamA || !m.teamB) { toast('Tim belum tersedia (menunggu round sebelumnya)', 'warning'); return; }
...
window.saveBO3 = function (matchId) {
  ...
  if (!valid) { toast('Skor BO3 tidak valid (2-0, 2-1, 0-2, 1-2)', 'error'); return; }
...
window.advanceBracket = function (data) {
  ...
  if (br.sf && br.final && br.bronze) {
    ...
    const sf1Loser = sf1?.winner ? (sf1.winner === sf1.teamA ? sf1.teamB : sf1.teamA) : null;
    const sf2Loser = sf2?.winner ? (sf2.winner === sf2.teamA ? sf2.teamB : sf2.teamA) : null;
  ...
  wrap.querySelectorAll('.timeline-date-input').forEach(input => {
    input.addEventListener('change', () => {
      ...
      if (idx !== -1) {
        ...
        if (preview) {
          preview.innerHTML = tempTl.map(stage => `
            <div class="timeline-track-item">
              <div class="timeline-track-dot">${stage.icon}</div>
              <div class="timeline-track-content">
                <div class="timeline-track-date">${formatDateID(stage.date)}</div>
...
function resetAll() {
  ...
  const emptyData = { teams: [], matches: [], bracket: {}, timeline: DEFAULT_TIMELINE };
  saveData(emptyData);
  renderAdmin();
...
function simulateAll() {
  ...
  data.matches.forEach(m => {
    if (Math.random() > 0.5) { m.scoreA = 1; m.scoreB = 0; }
  ...
  const simulateRound = (round) => {
    round.forEach(m => {
      ...
      if (Math.random() > 0.5) {
        m.scoreA = 2; m.scoreB = Math.random() > 0.5 ? 1 : 0; m.winner = m.teamA;
      } else {
        m.scoreB = 2; m.scoreA = Math.random() > 0.5 ? 1 : 0; m.winner = m.teamB;
      }
      m.played = true;
    });
  };
...
function getTeamStats(teamId, data) {
  ...
  data.matches.forEach(match => {
    if (match.played && (match.teamA === teamId || match.teamB === teamId)) {
      ...
      if (ts > os)       { wins++;   points += 1; }
      else if (ts === os) { draws++; }
      else                { losses++; points -= 1; }
    }
  });
...
function exportToWord() {
  ...
  a.href = url; a.download = `mlwc-export-${Date.now()}.doc`; a.click();
  URL.revokeObjectURL(url);
  toast('Data berhasil diexport ke Word', 'success');
}
...
function exportToPDF() {
  ...
  printWindow.document.write(`<!DOCTYPE html><html><head><title>MLWC Tournament Report</title>
    <style>
      ...
      .winner-cell { background:#e8f5e9; font-weight:bold; }
      .champion-row { background: gold; font-weight:bold; }
      h2 { margin-top:30px; color:#0066ff; }
      h3 { margin-top:20px; color:#333; }
      @media print { .section { page-break-inside: avoid; } }
    </style></head><body>${html}<p style="text-align:center;color:#999;margin-top:40px">Generated by MLWC Admin Dashboard • ${new Date().toLocaleString()}</p></body></html>`);
  printWindow.document.close();
  printWindow.print();
  toast('Silakan gunakan Print to PDF pada dialog print', 'success');
}

function generateExportHTML(data) {
  const groups = {};
  GROUPS.forEach(g => { groups[g] = []; });
  data.teams.forEach(t => { if (groups[t.group]) groups[t.group].push(t); });
  Object.keys(groups).forEach(g => {
    groups[g].sort((a, b) => getTeamStats(b.id, data).points - getTeamStats(a.id, data).points);
  });
  const matchesByGroup = {};
  GROUPS.forEach(g => { matchesByGroup[g] = []; });
  data.matches.forEach(m => { if (matchesByGroup[m.group]) matchesByGroup[m.group].push(m); });
  const tl = getTimeline(data);

  let html = `<div style="text-align:center;margin-bottom:30px"><h1>🏆 MLWC TOURNAMENT REPORT 🏆</h1><p>Generated: ${new Date().toLocaleString()}</p></div>`;

  html += `<h2>🗓️ TIMELINE TURNAMEN</h2><table><thead><tr><th>Tanggal</th><th>Tahap</th><th>Keterangan</th></tr></thead><tbody>`;
  tl.forEach(s => { html += `<tr><td>${formatDateID(s.date)}</td><td>${s.icon} ${s.label}</td><td>${s.desc}</td></tr>`; });
  html += `</tbody></table>`;

  html += `<h2>📊 GROUP STANDINGS</h2><p>Sistem Poin: Win = +1 | Draw = 0 | Lose = -1</p>`;
  for (const g of GROUPS) {
    if (!groups[g].length) continue;
    html += `<h3>Group ${g}</h3><table><thead><tr><th>Rank</th><th>Team</th><th>Tag</th><th>M</th><th>W</th><th>D</th><th>L</th><th>Poin</th></tr></thead><tbody>`;
    groups[g].forEach((t, i) => {
      const s = getTeamStats(t.id, data);
      html += `<tr ${i === 0 ? 'style="background:#e8f5e9"' : ''}><td>${i + 1}</td><td>${t.name}</td><td>${t.tag}</td><td>${s.played}</td><td>${s.wins}</td><td>${s.draws}</td><td>${s.losses}</td><td><b>${s.points}</b></td></tr>`;
    });
    html += `</tbody></table>`;
  }
  ...
  if (data.bracket?.r16?.length) {
    ...
    const rounds = [
      ...
      { key: 'bronze', label: '🥉 Perebutan Juara 3' },
      { key: 'final',  label: '🏆 Grand Final' },
    ];
    rounds.forEach(r => {
      const matches = data.bracket[r.key] || [];
      if (!matches.some(m => m.teamA)) return;
      html += `<h3>${r.label}</h3><table><thead><tr><th>Team A</th><th>Score</th><th>Team B</th><th>Winner</th></tr></thead><tbody>`;
      matches.forEach(m => {
        if (!m.teamA && !m.teamB) return;
        const a = teamById(m.teamA, data);
        const b = teamById(m.teamB, data);
        const w = teamById(m.winner, data);
        html += `<tr${r.key === 'final' ? ' class="champion-row"' : ''}><td>${a?.name || 'TBD'}</td><td><b>${m.played ? `${m.scoreA}-${m.scoreB}` : '-'}</b></td><td>${b?.name || 'TBD'}</td><td class="winner-cell">${w?.name || '-'}</td></tr>`;
      });
      html += `</tbody></table>`;
    });
  }
...
function regenerateSchedule() {
  ...
  const data = getData();
  if (!data.teams.length) { toast('Tidak ada tim. Tambah tim terlebih dahulu.', 'error'); return; }
  data.matches = generateGroupSchedule(data.teams);
  saveData(data);
  renderAdmin();
...
function autoAssignGroups(entries) {
  ...
  bucketList.forEach(([email, bucket]) => {
    ...
    bucket.forEach((entry, slotIndex) => {
      ...
      let picked = targetBlock.find(g =>
        groupSlots[g] < maxPerGroup && !usedSet.has(g)
      );
      ...
      if (!picked) {
        picked = [...BLOCK_1, ...BLOCK_2].reduce((best, g) =>
          groupSlots[g] < groupSlots[best] ? g : best
        , 'A');
      }
...
function openImportPreviewModal(entries, fileName, hasEmail) {
  ...
  const blockColor = g => ['A','B','C','D'].includes(g)
    ...
    : { bg: 'rgba(0,200,100,0.15)', text: '#00c853', label: 'Blok 2 · 7 Jun' };

  openModal('📥 Preview Import Tim dari Excel', `
