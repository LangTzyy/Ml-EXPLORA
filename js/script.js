...
const DEFAULT_TIMELINE = [
  {
    id: "group-ad",
    icon: "⚔️",
    label: "Babak Grup A–D",
    desc: "Round robin Grup A, B, C, D",
    date: "2026-06-06",
    cssClass: "stage-group-ad",
  },
  {
    id: "group-eh",
    icon: "⚔️",
    label: "Babak Grup E–H",
    desc: "Round robin Grup E, F, G, H",
    date: "2026-06-07",
    cssClass: "stage-group-eh",
  },
  {
    id: "r16",
    icon: "⚡",
    label: "Babak 16 Besar",
    desc: "Round of 16 (BO3)",
    date: "2026-06-13",
    cssClass: "stage-r16",
  },
  {
  ...
  },
  {
    ...
    desc: "Match antara 2 tim yang kalah di Semifinal (BO3)",
    date: "2026-06-21",
    cssClass: "stage-bronze",
  },
  {
    id: "final",
    icon: "🏆",
    label: "Grand Final",
    desc: "Perebutan Juara 1 & 2 (BO3)",
    date: "2026-06-21",
    cssClass: "stage-final",
  },
];
...
function advanceBracket(data) {
  ...
  if (!sf1?.winner || !sf2?.winner) {
    br.final[0].winner = null;
    br.final[0].scoreA = 0;
    br.final[0].scoreB = 0;
    br.bronze[0].winner = null;
    br.bronze[0].scoreA = 0;
    br.bronze[0].scoreB = 0;
  }
...
function renderSchedule() {
  ...
  if (!list.length) {
    wrap.innerHTML = emptyState(
      ...
      "Lihat hasil di bagian Hasil Pertandingan.",
    );
    return;
  }
  wrap.innerHTML = list
    .slice(0, 60)
    .map((m) => matchCardHtml(m, data, tl))
    .join("");
}
...
function renderResults() {
  ...
  if (!stageF || stageF === "group") {
    ...
    if (groupMatches.length) {
      html += groupMatches
        .map((m) => matchCardHtml(m, data, tl, true))
        .join("");
...
function renderStats() {
  ...
  const runner = finalMatch?.winner
    ? teamById(
    ...
    : null;
  const bronze3 = bronzeMatch?.winner
...
function startCountdown() {
  ...
  const tick = () => {
    ...
    if (labelEl)
      labelEl.textContent = stage.target
        ? "Menuju " + stage.label
